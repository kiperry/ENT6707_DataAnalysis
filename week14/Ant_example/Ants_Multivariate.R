############################################################################
# ENT 6707 Entomological Techniques and Data Analysis
#
...
# LTER Package ID: knb-lter-hfr.118.35
# doi:10.6073/pasta/3c4ded09a76203eec9e157e09309079f

# data are available online, so we can pull from the website
ants <- read.csv(file="https://pasta.lternet.edu/package/data/eml/knb-lter-hfr/118/35/90ca76917fe458ee78390c30de46afe5", 
...
ant.matrix <- dcast(ants, year + block + plot + treatment + trap.type ~ code, sum, value.var = "abundance")
str(ant.matrix)
...
# Let's subset the data to focus on pitfall trap data from years 2014 and 2015
library(tidyverse)

yr1 <- ant.matrix %>% filter(year == "2014") %>% droplevels()
yr2 <- ant.matrix %>% filter(year == "2015") %>% droplevels()
...
# here, we are using ant abundance data and calculating the bray-curtis dissimilarity matrix
dis.matrix <- vegdist(ants3[,6:35], method = "bray")
dis.matrix
...
nmds.ants$points # you can pull out the points that are plotted

# plot the NMDS model
ordiplot(nmds.ants, disp = "sites", type = "n", xlim = c(-1.5, 1.5), ylim = c(-1, 1))
...
legend("bottomleft", legend = c("Hemlock", "Girdled", "Logged", "Hardwood"),
...
ants4 <- ants3[,6:35] # save as a new object
...
# plot the NMDS model
ordiplot(nmds.ants.pa, disp = "sites", type = "n", xlim = c(-1.5, 1.5), ylim = c(-1.3, 1.3))
...
# cover: percent cover; values represent percent cover to the nearest 1%. Values less than 1% are listed as 0.5%
...
yr1.herb <- herb.matrix %>% filter(year == "2014") %>% droplevels()
yr2.herb <- herb.matrix %>% filter(year == "2015") %>% droplevels()
...
# two ways to do this
# option 1
plot(herb3[,5:25], pch = 19)
cor(herb3[,5:23], method = c("pearson"), use = "complete.obs") # not the easiest to interpret since we have so many plant species

library(ggplot2)
library(GGally)
cp <- ggpairs(herb3[,5:25], upper = list(continuous = wrap("cor", size = 2.5, color = "black")))
...
ants.rda <- rda(ants3[,6:35] ~ ., herb3[,5:25])
ants.rda
summary(ants.rda)
plot(ants.rda)

# model selection procedure
mod0.rda <- rda(ants3[,6:35] ~ 1, herb3[,5:25])
...
herb_vectors_rda <- envfit(ants.rda.red, herb3[, c(16,19)])

ordiplot(ants.rda.red, display = c('si', 'cn'), type = 'n', xlim = c(-12, 12), ylim = c(-12, 12))
...
# model selection procedure
mod0.cca <- cca(ants3[,6:35] ~ 1, herb3[,5:25])
...
herb_vectors_cca <- envfit(ants.cca.red, herb3[, c(7:9,19,21)])
...
DCA
# DCA1 axis length is less than 4, so RDA is the more appropriate analysis
