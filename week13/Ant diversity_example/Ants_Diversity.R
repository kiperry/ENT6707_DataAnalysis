...
# I've also provided the data as an excel file in case the link above does not work.
...
# one column for abundance of those species. Each row represents a species abundance
# at a site for a given sampling method (i.e., trap type). We want the data represented in 'wide format'.
# So, we want each species to have a separate column that has the abundance of the species at different sites
# represented in rows.
...
ants3$totabund <- rowSums(ants3[6:20])
colSums(ants3[6:20])
colSums(ants3[21])

# let's graph species abundance again
barplot(sort(colSums(ants3[,6:20]),decreasing=T), col="seagreen",
        xlab="", ylab="Abundance", las = 2, ylim = c(0, 800))
...
indval <- multipatt(ants3[,6:20], ants3[,4], duleg = TRUE, control = how(nperm=999)) # alpha = 0.05 is the default
...
boxplot(sp.rich ~ trap.type, data = ants3,
        xlab = "", ylab = "Ant species richness")
stripchart(sp.rich ~ trap.type, data = ants3, pch = 19, add = TRUE,
           vertical = TRUE, method = "jitter", jitter = 0.2)
...
plot(sp.girdled, pch = 19, col = "#481567FF", xvar = c("individuals"), lty = 4, lwd = 4,
     ylab = "Species Richness", xlab = "Number of Individuals", xlim = c(0, 400), ylim = c(0, 15))
