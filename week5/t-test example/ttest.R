...
# Study looks at the effects of temperature on herbivory rates of different
...
# Load the data
herb <- read.csv("./week5/t-test example/Feeding_Assays.csv")
...
summary(leps)
# relative growth rate (RGR) is growth rate relative to size. A rate of growth per unit time,
# as a proportion of its size at that moment in time.
...
boxplot(Herb_RGR ~ Temperature, data = leps.rgr, col = c("seagreen2", "steelblue2"),
        ylab = "Relative Growth Rate", xlab = "Temperature (C)")
