...
#
# Simple linear regression example: Part 2
#
...
# Load the data
growth <- read.csv("./week8/Simple linear regression_example/data_larval_growth.csv")
...
# in part 1, we subset the data to pull out data associated with the high-quality
# nutritional treatment. For part 2, let's pull out data for the low-quality treatment
beetle_low <- beetle %>% filter(nutritional.treatment == "low-quality") %>% droplevels()
levels(beetle_low$nutritional.treatment)

str(beetle_low)

plot(weight.adult ~ weight.L3, data = beetle_low, pch = 19)

beetle_mod2 <- lm(weight.adult ~ weight.L3, data = beetle_low)
summary(beetle_mod2)
anova(beetle_mod2)

par(mfrow = c(2, 2))
plot(beetle_mod2, pch = 19)
plot(beetle_mod2, which = 1:4, pch = 19)

par(mfrow = c(1, 1))
plot(weight.adult ~ weight.L3, data = beetle_low, pch = 19, cex = 1.3,
     xlab = "Weight of Third Larval Instar (g)",
     ylab = "Weight of Adult (g)")
abline(beetle_mod2)

fitted(beetle_mod2) # returns the fitted values (the Y values that you would expect
# for the given X values according to the best fit line)

resid(beetle_mod2) # returns the residuals
